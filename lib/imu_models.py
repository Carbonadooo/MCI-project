...
class MW2StackRNNPooling9Ch(pl.LightningModule):
    """
    9-channel IMU encoder (accelerometer + gyroscope + magnetometer)
    
    Adds a learnable 9→6 channel projection layer before the standard MW2 architecture.
    This allows using all 9 IMU channels while maintaining compatibility with 
    the pre-trained 6-channel model architecture.
    
    Input: [N x 9 x n_steps] (9 channels: accel_xyz + gyro_xyz + mag_xyz)
    Output: [N x size_embeddings]
    """
    def __init__(self, input_dim=32, size_embeddings: int = 128):
        super().__init__()
        self.name = MW2StackRNNPooling9Ch
        
        # Channel projection: 9 → 6 (learnable)
        self.channel_projection = torch.nn.Conv1d(
            in_channels=9,
            out_channels=6,
            kernel_size=1,  # 1x1 conv for channel-wise projection
            bias=True
        )
        
        # Rest of the network (same as MW2StackRNNPooling)
        self.net = torch.nn.Sequential(
            torch.nn.GroupNorm(2, 6),
            Block(6, input_dim, 10),
            Block(input_dim, input_dim, 5),
    ...
    def forward(self, batch):
        # Project 9 channels → 6 channels
        batch = self.channel_projection(batch)
        # Pass through the rest of the network
        return self.net(batch)[1][0]


class MW2StackRNNPooling18Ch(pl.LightningModule):
    """
    18-channel IMU encoder (9 IMU channels + 9 rotation matrix features)
    
    Input: [N x 18 x n_steps] where:
        - channels 0-2: accelerometer (ax, ay, az)
        - channels 3-5: gyroscope (gx, gy, gz)
        - channels 6-8: magnetometer (mx, my, mz)
        - channels 9-17: flattened 3x3 rotation matrix (9 values)
    
    The rotation matrix features are computed from the raw IMU data and provide
    explicit orientation information to complement the raw sensor readings.
    
    Output: [N x size_embeddings]
    """
    def __init__(self, input_dim=32, size_embeddings: int = 128):
        super().__init__()
        self.name = MW2StackRNNPooling18Ch
        
        # Channel projection: 18 → 6 (learnable)
        # This learns to combine raw IMU + rotation features
        self.channel_projection = torch.nn.Conv1d(
            in_channels=18,
            out_channels=6,
            kernel_size=1,  # 1x1 conv for channel-wise projection
    ...
    def forward(self, batch):
        # Project 18 channels → 6 channels
        batch = self.channel_projection(batch)
        # Pass through the rest of the network
        return self.net(batch)[1][0]


class MW2StackRNNPooling21Ch(pl.LightningModule):
    """
    21-channel IMU encoder (9 IMU + 9 rotation features + 3 world-frame linear acceleration)
    
    Input: [N x 21 x n_steps] where:
        - channels 0-2: accelerometer (ax, ay, az) - body frame
        - channels 3-5: gyroscope (gx, gy, gz)
        - channels 6-8: magnetometer (mx, my, mz)
        - channels 9-17: flattened 3x3 rotation matrix (R11-R33)
        - channels 18-20: world-frame linear acceleration (gravity-compensated)
    
    The world-frame acceleration is computed by:
        1. Rotating body-frame acceleration to world frame using R(t)
        2. Removing gravity vector [0, 0, -9.81]
    
    This provides motion information independent of sensor orientation.
    
    Output: [N x size_embeddings]
    """
    def __init__(self, input_dim=32, size_embeddings: int = 128):
        super().__init__()
        self.name = MW2StackRNNPooling21Ch
        
        # Channel projection: 21 → 6 (learnable)
        # Learns to combine raw IMU + rotation + world-frame features
        self.channel_projection = torch.nn.Conv1d(
            in_channels=21,
            out_channels=6,
            kernel_size=1,  # 1x1 conv for channel-wise projection
    ...
    def forward(self, batch):
        # Project 21 channels → 6 channels
        batch = self.channel_projection(batch)
        # Pass through the rest of the network
        return self.net(batch)[1][0]