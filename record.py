...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='Example: python record.py --folder_name test_data --task_name pick_and_place --webcam_port 2 --serial_port /dev/tty.usbmodem1101 --duration 15'
    )
    ...
    parser.add_argument(
        ...
        help='Serial port for IMU device (macOS example: /dev/tty.usbmodem1101)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help='IMU sampling frequency in Hz (default: 200)'
    )
    ...
    print(f"Duration: {args.duration}s | IMU: {args.imu_freq} Hz | Camera: {args.camera_freq} FPS")
    print(f"Output directory: {output_dir}")
    print(f"Output file: {output_hdf5}")

    try:
        # Import heavy dependencies only when running (so --help works without them)
        ...
        collector = DataCollector(
            ...
            imu_freq=args.imu_freq,  # Hz
            camera_freq=args.camera_freq,  # FPS
        ...
        success = collector.start_recording(duration=args.duration)
